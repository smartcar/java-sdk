...
public class SmartcarTest extends PowerMockTestCase {
  ...
  public void testGetUser() throws Exception {
    ...
    PowerMockito.when(System.getenv("SMARTCAR_API_ORIGIN"))
  ...
  public void testVehicles() throws Exception {
    ...
    MockResponse response =
        new MockResponse()
            .setBody(
                "{ \"paging\": {\"count\": 1, \"offset\": 0 }, \"vehicles\": [\""
                    + vehicleId
                    + "\"] }")
            .addHeader("sc-request-id", this.sampleRequestId);
    TestExecutionListener.mockWebServer.enqueue(response);
  ...
  public void testGetCompatibility() throws Exception {
    ...
    Assert.assertEquals(
        req.getPath(), "/v2.0/compatibility?vin=1234&scope=read_odometer&country=US");
  }
  ...
  public void testGetCompatibilityWithOptions() throws Exception {
    ...
    Assert.assertEquals(
        req.getPath(),
        "/v1.0/compatibility?vin=1234&scope=read_odometer&country=GB&flags=foo%3Abar%20test%3Atrue&mode=test&testModeCompatibilityLevel=hello");
  }
